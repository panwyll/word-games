...
import { generateConnectionsPuzzle } from '@/lib/connections-engine';
...
export default function ConnectionsArchive() {
  ...
  if (selectedOffset !== null) {
    ...
    const puzzle = generateConnectionsPuzzle(targetSeed);
    const date = new Date();
    ...
    return (
      <div>
        ...
        <ConnectionsGame overridePuzzle={puzzle} />
      </div>
    );
  }
      ...
      {Array.from({ length: ARCHIVE_DAYS }, (_, i) => {
        ...
        return (
          ...
          >
            ...
            <span className="text-xs text-gray-400">16 words →</span>
          </button>
        );
      })}
    </div>
